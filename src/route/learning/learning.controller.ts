...
export class LearningController {
  ...
  async getStudySetStats(
    ...
    const stats = await this.learningService.getStudySetStats(
      studySetId,
      user.userId,
    );
  ...
  async updateVocabularyProgress(
    ...
    const data = await this.learningService.updateVocabularyProgress(
      user.userId,
      vocabId,
      result,
    );
  ...
  async getUserProgress(@ActiveUser() user: TokenPayload) {
    return new UserProgressResponseDto(
      await this.learningService.getUserProgress(user.userId),
    );
  }
}
