...
import {
  CambridgeEnglishResult,
  CambridgeEnglishPronunciation,
  CambridgeEnglishEntry,
  CambridgeEnglishMeaning,
...
export async function crawlCambridgeDictionary(
  word: string,
...
export async function crawlCambridgeEnglishDictionary(
  word: string,
): Promise<CambridgeEnglishResult> {
  // Normalize word
  word = word.trim().toLowerCase();
  if (!word) {
    return {
  ...
  if (cache[cacheKey]) {
    console.log(
      `Found '${word}' in English dictionary cache. Returning immediately.`,
    );
  ...
  try {
    ...
    if (final_data.status === "success") {
      console.log(
        `Successfully crawled '${word}' from English dictionary. Saving to cache.`,
      );
...
export const processVocabularyData = (
  ...
  if (englishData.status === "success") {
    ...
    cefrLevel = englishEntry?.meanings[0]?.cefr_level.substring(0, 2);
    if(!Object.values(CefrLevel).includes(cefrLevel)) {
